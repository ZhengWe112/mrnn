...
class NBEATSStack(nn.Module):
    def __init__(
        ...
        nbeats_block = NBEATSGenericBlock(units, thetas_dim, num_block_layers, backcast_length, forecast_length, dropout)
        bl_stack = []
        for _ in range(stack_layers):
            bl_stack.extend([nbeats_block])
    ...
    def forward(self, x):
        y = []
        for idx, block in enumerate(self.bl):
            xt, yt = block(x)
            x = x - xt
            y.append(yt)
...
class NBEATS(nn.Module):
    def __init__(
        ...
        for _ in range(layers):
            st_stack.extend([NBEATSStack(units, thetas_dim, num_block_layers,
                                         backcast_length, forecast_length, dropout, stack_layers)])
    ...
    def forward(self, x):
        y = []
        for idx, block in enumerate(self.st):
            xt, yt = block(x)
            x = x - xt
            y.append(yt)
